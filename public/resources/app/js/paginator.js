...
zewa.paginator = (function($){
    ...
    var renderResults = function(paginator, results) {
        ...
        if(results === "") {
            ...
            if(paginator.initialRun === true) {
                ...
                paginator.container.html(paginator.wrapper.find('tbody').html());
            } else {
                paginator.container.html(paginator.emptyResultSet);
            }
        } else {
    ...
    var preparePaginatorSearch = function(paginator) {
        ...
        paginator.wrapper.on('click', '.paginator-submit', function (e) {
            //is form valid
            ...
            $(paginator.searchObject).each(function (key, value) {
                if($(value).val() === null || $(value).val().trim() === "") {
                    if($(value).attr('required') !== undefined) {
                        $(value).parent().css({'border':'1px solid red', 'border-radius': '3px'});
                        readyToSearch = false;
                    }
                } else {
                    paginator.searchData = true;
                    $(value).parent().css({'border':'none'});
                }
            });

            if(readyToSearch === true) {
                searchQuery(paginator);
            }
        });

        setTimeout(function(){
            if(paginator.autosearch === true || paginator.autosearch === undefined) {
    ...
    var searchQuery = function(paginator) {
        paginator.page = 1;
    ...
    var Paginator = function(alias, wrapper) {
        ...
        this.autosearch = wrapper.data('paginate-autosearch');
        this.form = wrapper.find('.paginate-form');
